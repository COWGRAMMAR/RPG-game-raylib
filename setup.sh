...
# Install mpv development libraries (video playback backend)
install_mpv() {
  write_step "Checking for mpv development libraries..."

  # Check if already installed via pkg-config
  if command -v pkg-config &> /dev/null && pkg-config --exists mpv 2>/dev/null; then
    write_step "mpv development libraries already installed"
    return
  fi

  if ! command -v sudo &> /dev/null; then
    write_err "sudo is required but not available for mpv installation"
    exit 1
  fi
  ...
  if command -v pkg-config &> /dev/null && pkg-config --exists mpv 2>/dev/null; then
    write_step "mpv development libraries installed successfully"
  else
